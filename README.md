...
The API is single-threaded and is intended to be used in a single process with no threads, or via a single I/O manager thread. I/O requests submitted via `ioqueue_pread` or `ioqueue_pwrite` are asynchronous and will not begin to execute until after the next call to `ioqueue_reap()`, which blocks.
